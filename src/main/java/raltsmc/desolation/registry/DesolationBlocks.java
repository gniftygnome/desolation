...
public final class DesolationBlocks {
    ...
    static void init() {
        ...
        SCORCHED_TUFT = DesolationRegistries.register("scorched_tuft", new ScorchedTuftBlock(FabricBlockSettings.copyOf(Blocks.GRASS).mapColor(MapColor.GRAY).sounds(BlockSoundGroup.CROP)));
        ...
        CHARRED_SAPLING = DesolationRegistries.register("charred_sapling", new CharredSaplingBlock(new CharredSaplingGenerator(() -> DesolationConfiguredFeatures.TREE_CHARRED), FabricBlockSettings.copyOf(Blocks.OAK_SAPLING)));
        ...
        CHARRED_PRESSURE_PLATE = DesolationRegistries.register("charred_pressure_plate", new DesolationPressurePlateBlock(ActivationRule.EVERYTHING, FabricBlockSettings.copyOf(Blocks.OAK_PRESSURE_PLATE).mapColor(MapColor.GRAY)));
        CHARRED_TRAPDOOR = DesolationRegistries.register("charred_trapdoor", new DesolationTrapdoorBlock(FabricBlockSettings.copyOf(Blocks.OAK_TRAPDOOR).mapColor(MapColor.GRAY).strength(3.0f).allowsSpawning((state, world, pos, entityType) -> false)));
        ...
        CHARRED_FENCE_GATE = DesolationRegistries.register("charred_fence_gate", new FenceGateBlock(FabricBlockSettings.copyOf(Blocks.OAK_FENCE_GATE).mapColor(MapColor.GRAY), WoodType.OAK));
        CHARRED_BUTTON = DesolationRegistries.register("charred_button", new DesolationWoodenButtonBlock(FabricBlockSettings.copyOf(Blocks.OAK_BUTTON).mapColor(MapColor.GRAY)));
        CHARRED_DOOR = DesolationRegistries.register("charred_door", new DesolationDoorBlock(FabricBlockSettings.copyOf(Blocks.OAK_DOOR).mapColor(MapColor.GRAY)));
    }
}
